...
def add_cash():
    ...
    if request.method == "GET":
        ...
        user_cash_db = db.execute("SELECT cash FROM users WHERE id = :id", id=user_id)  # Correct indentation
        user_cash = user_cash_db[0]["cash"]
...
def sell():
    ...
    if request.method == "GET":
        user_id = session["user_id"]
        # Corrected 'transaction' to 'transactions'
        symbols_user = db.execute("SELECT symbol FROM transactions WHERE user_id = :id GROUP BY symbol HAVING SUM(shares) > 0", id=user_id)
        return render_template("sell.html", symbols=[row["symbol"] for row in symbols_user])  # Fixed variable name from symbols to symbols_user
        ...
        # Updated this line to correctly reference user_id and the symbol
        ...
        # Check if there are any shares found
        if len(user_shares) == 0 or user_shares[0]["shares"] is None:
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?,?,?,?,?)", user_id, stock["symbol"], -shares, stock["price"], date)  # Use -shares for selling
